...
mongoose.connect("mongodb+srv://wimpywarlord:wimpycool@cluster0-5xhfl.mongodb.net/test?retryWrites=true&w=majority", { useNewUrlParser: true, useUnifiedTopology: true, dbName: 'goodmorningGirlfriend' }, function (err, res) {
    if (err) {
        console.log(err);
    }
    else {
...
router.post("/", function (req, res) {
    ...
    user.create({ email: req.body.email, name_of_sender: req.body.name_of_sender, days: req.body.days, }, function (err, yolo) {
        if (err) {
